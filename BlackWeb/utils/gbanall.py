...
def globals_init():
    try:
        ...
        sql = import_module("BlackWeb.database.SQL.gban_sql")
        sql2 = import_module("BlackWeb.database.SQL.gmute_sql")
...
async def gban_user_all(client, message):
    ...
    if user_id:
        try:
            ...
            return await ren.edit("`Please specify a valid user!`")

    if sql.is_gbanned(user.id):
        return await ren.edit(
            f"[Gbanned](tg://user?id={user.id}) **it's already on the gbanned list**"
        )
