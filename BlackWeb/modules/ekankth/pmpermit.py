...
from BlackWeb.database.SQL.globals import addgvar, gvarstatus
...
async def incomingpm(client: Client, message: Message):
    try:
        from BlackWeb.database.SQL.globals import gvarstatus
        from BlackWeb.database.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from BlackWeb.database.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
    ...
    if message.chat.id in DEVS:
        try:
            ...
            await client.send_message(
                message.chat.id,
                f"<b>Receiving Messages!!!</b>\n{message.from_user.mention} <b>Developer detected BlackWeb-Userbot</b>",
...
async def approvepm(client: Client, message: Message):
    try:
        from BlackWeb.database.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from BlackWeb.database.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from BlackWeb.database.SQL.globals import addgvar
...
async def setpmpermit(client: Client, cust_msg: Message):
    ...
    try:
        import BlackWeb.database.SQL.globals as sql
    except AttributeError:
        await cust_msg.edit("**Running on Non-SQL mode!**")
        return
    ren = await cust_msg.edit("`Processing...`")
