...
from partharjun.helper.sticker import *
...
@ren.on_message(filters.command(["sticker", "stickers"], cmd) & filters.me)
async def cb_sticker(client: Client, message: Message):
    query = get_text(message)
    if not query:
        return await message.edit_text("**Enter the name of the Sticker Pack!**")
    xx = await message.edit_text("<code>Searching sticker packs...</code>", parse_mode=None)
    text = requests.get(f"https://combot.org/telegram/stickers?q={query}").text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await xx.edit_text("**Cannot Find Sticker Pack**", parse_mode=None)
    ...
    await xx.edit_text(reply, parse_mode=None)
